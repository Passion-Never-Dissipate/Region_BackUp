...
> [!TIP]
> 
> 尽可能的使用最新版本，以避免潜藏的问题，升级版本后虽然配置文件能自动升级，但请尽可能检查下配置文件是否无误请
> 
...
4.在v1.8.0版本插件新增了静态备份功能(即不会被替换的备份)，静态备份采用独立文件夹存储在./rb_static，与之相对的普通备份存储在./rb_multi，具体见指令说明。

5.由于插件在备份时会保存区域的方块数据、实体数据和兴趣点数据，因此可能会有玩家用此来刷取物品，因为插件只会备份与区域有关的数据，而不会与其他数据产生交互。

6.尽量不要在插件进行备份回档操作时重载插件。
